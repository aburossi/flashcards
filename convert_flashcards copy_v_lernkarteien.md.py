...
def save_flashcards(subject, flashcards):
    ...
    The JSON file is named using the subject name.
    """
...
def process_root_folder(root_folder):
    """
    Recursively goes through the subfolders of the given root folder,
    looking for MD files named 'lernkarteien.md'.
    If found, processes the file and saves flashcards as a JSON file named after the subfolder.
    """
    for dirpath, dirnames, filenames in os.walk(root_folder):
        for file in filenames:
            if file.lower() == "lernkarteien.md":
                ...
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    if flashcards:
                        ...
                        print(f"No valid flashcards found in {file_path}.")
    run_update_subjects()
...
def choose_root_folder():
    ...
    if root_folder:
        ...
        messagebox.showinfo("Success", "Flashcards from the root folder have been successfully converted and saved.")
...
def create_gui():
    ...
    # Tab 3: Choose Root Folder
    tab3 = ttk.Frame(tab_control)
    tab_control.add(tab3, text='Choose Root Folder')
    ...
    # --- Tab 3: Choose Root Folder ---
    root_folder_button = ttk.Button(tab3, text="Choose Root Folder", command=choose_root_folder)
    root_folder_button.pack(pady=20)
